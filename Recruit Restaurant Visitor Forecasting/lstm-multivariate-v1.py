...
def create_dataset(dataset: np.ndarray, look_back: int=1) -> (np.ndarray, np.ndarray):
    ...
    data_x = np.empty(shape=[0, n_features])
    data_y = []
    for i in range(len(dataset) - look_back - 1):
        a = dataset[i:(i + look_back)]
        data_x = np.append(data_x, np.array(a), axis=0)
        data_y.append(dataset[i + look_back, n_features - 1])
...
def model_lstm(look_back: int, batch_size: int=1, features=1):
    model = Sequential()
    ...
    model.add(CuDNNLSTM(KERAS_NODES, batch_input_shape=(
        batch_size, look_back, features), stateful=True, return_sequences=False))
...
def make_forecast(model: Sequential, look_back_buffer: np.ndarray, test, batch_size: int=1):
    # https://github.com/gcarq/keras-timeseries-prediction/blob/master/main.py
    forecast_predict = np.empty((0, 1), dtype=np.float32)
    timesteps = len(test)
    cur_predict = model.predict(look_back_buffer, batch_size)
    n_features = test.shape[2]
    print('n_features', n_features)
    test[0, :, n_features - 1] = cur_predict
    for i in trange(timesteps, desc='predicting data\t', mininterval=1.0):
        # make prediction with current lookback buffer
        input_x = np.reshape(test[i], (test[i].shape[0], 1, test[i].shape[1]))
        cur_predict = model.predict(input_x, batch_size)
        # add prediction to result
        forecast_predict = np.concatenate(
            [forecast_predict, cur_predict], axis=0)
        if i < timesteps - 1:
            test[i + 1, :, n_features - 1] = cur_predict
...
def train_model(train_set, test_set):
    ...
    # print("Before shape:", data.shape[0], data.shape[1])
    ...
    train_x = np.reshape(
        train_x, (train_x.shape[0], look_back, train_x.shape[1]))
    ...
    test_set = np.reshape(
        test_set, (test_set.shape[0], look_back, test_set.shape[1]))
    forecast_predict = make_forecast(
        model, test_x[-1::], test_set, batch_size=batch_size)

    #================> Wait ----------------------     
    # invert dataset and predictions
    dataset = scaler.inverse_transform(dataset)
    train_x[:, :, n_features - 1] = train_predict
    train_x = scaler.inverse_transform(train_x)
    # train_predict = scaler.inverse_transform(train_predict)
    train_predict = train_x[:, :, n_features - 1]
...
if __name__ == "__main__":
    ...
    label = 'visitors'
    cols = ['holiday_flg', 'rs1_x', 'rv1_x', 'rs2_x', 'rv2_x',
            'rs1_y', 'rv1_y', 'rs2_y', 'rv2_y', 'visitors']
    # cols = [label]
    count = 0
    batch_size = 1
    look_back = 1
    ...
    # Build model
    model = model_lstm(look_back, batch_size=batch_size, features=len(cols))
    ...
    #     train_set = train[train.air_store_id == air_store_id][cols]
    #     test_set = test[test.air_store_id == air_store_id][cols]
    #     try:
    ...
    eval_output.to_csv(
        OUTPUT_DIR + '/' + today + '-multi-submission.csv', index=False, float_format='%.7f')
    end = time.time() - start
