...
class MercatiPriceSuggestion():
    def __init__(self, label, word2vec=0, model_choice=MODEL_CNN, model_choice2=None):
        ...
        self.embedding_features = [
            'name', 'item_description', 'brand_name', 'category_name']
        # self.embedding_features = ['name', 'item_description']
        self.special_features = 'item_description'
    ...
    def load_data(self):
        ...
        # train_data = pd.read_table(DATA_DIR + "/train.tsv", nrows=10000)
        # eval_data = pd.read_table(DATA_DIR + "/test.tsv", nrows=10000)
        train_data = pd.read_table(DATA_DIR + "/train.tsv")
        eval_data = pd.read_table(DATA_DIR + "/test.tsv")
    ...
    def preprocess_text(self, texts, preprocess=1):
        ...
        if preprocess == 1:
            ...
            new_texts = get_stem(texts, texts.index)
    ...
    def tokenize(self, texts):
        ...
        new_texts = tokenizer.texts_to_sequences(texts)
        max_sequence_length = np.amax(list(map(len, new_texts)))
        logger.debug("Sequence length:" + str(max_sequence_length))
        # pad documents to a max length
        new_texts = pad_sequences(new_texts, max_sequence_length)
        return vocab_size, max_sequence_length, new_texts

    def tokenize_and_ngram(self):
        self.combine_data['item_description'] = self.preprocess_text(
            self.combine_data['item_description'], preprocess=0)
        self.vocab_size = {}
        self.sequence_length = {}
        # Using multi-process to speed up, excluding: item_description because of big memory size
        features = self.embedding_features.copy()
        features.remove(self.special_features)
        p = Pool(processes=len(features))
        results = p.map(_tokenize_single, [
                        (key, self.combine_data[key]) for key in features])
        for result in results:
            key = result[0]
            self.vocab_size[key] = result[1]
            self.sequence_length[key] = result[2]
            self.combine_data[key] = result[3]

        # For item_description, use local processing
        key = self.special_features
        logger.debug("Tokenizing for " + key)
        self.vocab_size[key], self.sequence_length[key], self.combine_data[key] = self.tokenize(
            self.combine_data[key])
        logger.debug(key + ":Train shape after padding:" +
                     str(self.combine_data[key].shape))
        logger.debug("Vocabulary size:" + str(self.vocab_size))
        logger.debug("Sequence length:" + str(self.sequence_length))
        # Create n-gram
        if NGRAM_RANGE > 1:
    ...
    def prepare_data(self):
        ...
        self.eval_id = self.eval_data['test_id'].astype(int)
    ...
    def get_keras_data(self, dataset):
        ...
        X['num_vars'] = dataset[["shipping", 'item_condition_id']].values
        # X['num_vars'] = dataset[['item_condition_id']].values
        return X

    def buil_embbeding_layer(self, vocab_size, output_dim, input_length):
        if self.word2vec > 0:
    ...
    def build_model(self, X_train, X_test):
        ...
        if key_var in X_train:
            vocab_size = max(np.max(X_train['num_vars']), np.max(
                X_test['num_vars'])) + 1
        ...
        if self.model_choice == MODEL_FASTEXT:
            ...
            in_models = []
            for key in self.embedding_features:
                input, in_model = self.model_fasttext(
                    key, self.vocab_size[key], OUTPUT_DIM, self.sequence_length[key])
                inputs.append(input)
                in_models.append(in_model)
            if key_var in X_train:
                input, in_model = self.model_fasttext(
                    'num_vars', vocab_size, 5, X_train['num_vars'].shape[1])
                inputs.append(input)
                in_models.append(in_model)
        ...
        self.model.compile(optimizer=optimizer, loss=keras_rmse)
        # summarize the model
    ...
    def train_single_model(self):
        ...
        self.build_model(X_train, X_test)
        # Use Early-Stopping
