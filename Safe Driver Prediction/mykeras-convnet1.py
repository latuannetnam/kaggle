...
from keras.layers import Input, Dense, Dropout, BatchNormalization, Flatten, Conv1D, MaxPooling1D, GlobalMaxPooling1D, LSTM, CuDNNLSTM, Bidirectional, GlobalAveragePooling1D, Reshape
...
if __name__ == "__main__":
    ...
    # create model
    model = Sequential()
    model.add(Reshape((1, n_features), input_shape=(n_features,)))
    # model.add(Dropout(dropout, seed=random_state))
    for i in range(KERAS_LAYERS):
        model.add(Conv1D(KERAS_FILTERS, KERAS_KERNEL_SIZE, strides=KERAS_STRIDE, activation='relu'))
    ...
    callback_early_stopping2 = keras.callbacks.EarlyStopping(
        monitor='val_loss', patience=KERAS_EARLY_STOPPING, verbose=VERBOSE, mode='min')
    ...
    logger.debug('Best metric:' + str(callback_checkpoint.best))
