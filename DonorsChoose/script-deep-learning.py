...
session_conf = tf.ConfigProto(
    intra_op_parallelism_threads=6, inter_op_parallelism_threads=5)
from keras import backend

tf.set_random_seed(10003)
backend.set_session(tf.Session(
    graph=tf.get_default_graph(), config=session_conf))
...
def create_features(df):
    ...
    df['text'] = df.apply(lambda x: " ".join(x[col]
                                             for col in char_cols), axis=1)
    return df
...
num_features = ["teacher_number_of_previously_posted_projects",
                "total_price_x", "total_price_y", "total_price"]
...
def get_model3():
    ...
    model = Model(inputs=[input_cat, input_num,
                          input_words], outputs=predictions)
...
pred_test = model.predict(
    [X_test_cat, X_test_num, X_test_words], batch_size=2000)

test["project_is_approved"] = pred_test
test[['id', 'project_is_approved']].to_csv(
    "data-temp/gru_cnn_submission.csv", index=False)
print("Done")
